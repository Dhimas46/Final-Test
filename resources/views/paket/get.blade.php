      ...
      <div class="card-body">
        <table class="table table bordered">
          ...
          <tbody>
            ...
            <div class="modal modal fade" id="edit-Modal{{$row->id}}" tabindex="-1" role="dialog" aria-labelledby="label" aria-hidden="true">
              <div class="modal-dialog">
                <div class="modal-content">
                  ...
                  <div class="modal-body">
                    <form class="" action="{{url('paket/'.$row->id)}}" method="post">
                      @csrf
                      ...
                      <div class="row">
                        ...
                        <div class="col-md-6">
                          ...
                          <select class="form-control" name="satuan_id" >
                            <option value="{{$row->satuan_id}}">{{$row->satuan->satuan}}</option>
                      ...
                      <div class="form-group">
                        ...
                        <input type="text" name="cabang" value="{{$row->cabang}}" class="form-control" required>
                      </div>
                  </div>
            ...
            <tr>
              <td>{{$row->paket}}</td>
              <td>{{$row->berat}}&nbsp{{$row->satuan->satuan}}</td>
              <td>Rp @money($row->harga)</td>
              <td>{{$row->cabang}}</td>
              <td>{{$row->created_at}}</td>
              <td>
                <span class="badge badge-sm {{ ($row->status == '1')?'bg-gradient-success' : 'bg-gradient-secondary' }}">
              ...
              <td>
                <div class="btn-group">
                  ...
                  <div class="dropdown-menu text-left" role="menu">
                    ...
                    <a data-toggle="modal" data-target="#edit-Modal{{$row->id}}" class="dropdown-item" href="">&nbsp<i class="fa fa-info"></i>&nbsp &nbsp Detail</a>
                      </form>
                      <form class="d-inline" action="{{url('aktif-paket/'.$row->id)}}" method="post">
                      @csrf
                      @if($row->status == '0')
